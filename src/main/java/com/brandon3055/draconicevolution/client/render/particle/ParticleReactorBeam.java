...
//			float sizeTarget = 0.2F;
            ...
            float texV1 = MathHelper.sqrt_float(offsetX * offsetX + offsetY * offsetY + offsetZ * offsetZ) + ((float) particleAge + partialTick) * 0.01F * speed;
            ...
            length = MathHelper.sqrt_float(offsetX * offsetX + offsetY * offsetY + offsetZ * offsetZ) - (float) (reactor.getCoreDiameter() / 2D) - 0.3F;
    ...
    private void renderStabilizerEffect(Tessellator tessellator, TileReactorCore reactor, float offsetX, float offsetY, float offsetZ, float length, float partialTick) {
        ...
        drawBeam(tessellator, reactor, 1F, 0.355F, 0.8F, offsetX, offsetY, offsetZ, particleAge, partialTick, true, false, 0x00ffff);
        GL11.glPopMatrix();
        ...
        float s = 0.355F;
        drawBeam(tessellator, reactor, s / coreSize, coreSize, length - (float) (reactor.getCoreDiameter() / 2.5D), offsetX, offsetY, offsetZ, particleAge, partialTick, false, false, 0x00ffff);
        GL11.glPopMatrix();

        GL11.glPushMatrix();
        GL11.glTranslated(0, 0, -0.35);
        drawBeam(tessellator, reactor, 1F, 0.263F, 0.8F, offsetX, offsetY, offsetZ, particleAge, partialTick, true, true, 0xff6600);
        GL11.glPopMatrix();
        ...
        s = 0.263F;
        drawBeam(tessellator, reactor, s / coreSize, coreSize, length - 0.5F, offsetX, offsetY, offsetZ, particleAge, partialTick, false, true, 0xff6600);
        GL11.glPopMatrix();
    ...
    private void drawBeam(Tessellator tessellator, TileReactorCore reactor, float sizeOrigin, float sizeTarget, float length, float offsetX, float offsetY, float offsetZ, int tick, float partialTick, boolean reverseTransparency, boolean reverseDirection, int color) {
        ...
        float texV1 = MathHelper.sqrt_float(offsetX * offsetX + offsetY * offsetY + offsetZ * offsetZ) / 32.0F + ((float) tick + partialTick) * (reverseDirection ? -0.01F : 0.01F) * speed;
...
//		GL11.glRotatef((float)(-Math.atan2((double)f7, (double)offsetY)) * 180.0F / (float)Math.PI - 90.0F, 1.0F, 0.0F, 0.0F);
//
//		if (tile instanceof TileReactorStabilizer) renderStabilizerEffect(tessellator, offsetX, offsetY, offsetZ, length, partialTick);
//		else {
//			GL11.glShadeModel(GL11.GL_SMOOTH);
//			GL11.glTranslated(0, 0, 0.3);
//			//GL11.glPushMatrix();
