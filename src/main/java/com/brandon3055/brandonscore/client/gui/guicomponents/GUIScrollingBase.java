...
public abstract class GUIScrollingBase extends GUIBase {
	...
	public void handleMouseInput() {
		...
		if (i != 0) {
			...
			{
				if (c instanceof ComponentScrollingBase && c.isEnabled()) ((ComponentScrollingBase) c).handleScrollInput(i > 0 ? -1 : 1);
			}
		}
	}
	...
	protected void mouseClicked(int x, int y, int button) {
		...
		clickedY = y;
		if ((x-guiLeft+17) > 0 && (x-guiLeft+17) < 17 && clickedY-guiTop-20 > barPosition && clickedY-guiTop-20 < barPosition + 38) scrollPressed = true;
	}
	...
	protected void mouseClickMove(int x, int y, int button, long time) {
		...
		barMoved((double) barPosition / 247D);
	}
